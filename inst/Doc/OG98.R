...
data <- data.frame(read.spss
      ("../NLOG98_F_CMC2.sav",use.value.labels=TRUE))
# Add education
...
NLOG98  <- data.frame (ID=ID,born=born,start=born,end=interview,kerk=kerk,educ=educ,cohort=cohort,YearInt=trunc(1900+interview/12),idim=as.numeric(rep(1,nsample)),ns=as.numeric(ns),path=as.character(path),dates[,1:(max(ns)-1)],stringsAsFactors=FALSE)

attr(NLOG98,"format.date") <- "CMC"
param <- Parameters (NLOG98)
attr(NLOG98,"trans") <- param$tmat
save (NLOG98,file="NLOG98.RData")
