...
RS <- data.frame (ID=id,born=birth,start=as.Date(entry,"%d/%m/%Y"),end=as.Date(interview,"%d/%m/%Y"),cov=cov,idim=as.numeric(rep(1,length(id))),ns=as.numeric(ns),path=as.character(path),dates[,1:(max(ns)-1)],stringsAsFactors=FALSE)
attr(RS,"format.date") <- "%Y-%m-%d"
z<- Parameters (RS)
attr(RS,"trans") <- z$tmat
zz8 <- "/Users/franswillekens/Documents/R/0 0 MAC/Package/TEST.Biograph/Biographies RealSynthetic/"
save (RS,file=paste(zz8,"real_synthetic.RData",sep=""))
