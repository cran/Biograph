...
library(foreign)
# ---------------   Read data  ----------------
d.st <- data.frame(read.dta ("/../SHARE/Wave3Stata/sharew3_rel1_st.dta",convert.dates=TRUE,convert.underscore=TRUE))
d.rp <- data.frame(read.dta ("../SHARE/Wave3Stata/sharew3_rel1_rp.dta",convert.dates=TRUE,convert.underscore=TRUE))
d.ac <- data.frame(read.dta ("../SHARE/Wave3Stata/sharew3_rel1_ac.dta",convert.dates=TRUE,convert.underscore=TRUE))
d.re <- data.frame(read.dta ("/../SHARE/Wave3Stata/sharew3_rel1_re.dta",convert.dates=TRUE,convert.underscore=TRUE)) # retro employment + educ
d.rc <- data.frame(read.dta ("../SHARE/Wave3Stata/sharew3_rel1_rc.dta",convert.dates=TRUE,convert.underscore=TRUE)) # retro children
...
SHARE<- data.frame(ID=c(1:nsample),born=as.numeric(bb),start=as.numeric(bb),end=as.numeric(end),country=as.factor(d.st$country),IDc=IDc,cohort=bcohort,sex=as.factor(sex2),eduf=as.numeric(edu.f),job1=as.numeric(job.1.start),children=nchildren,idim=as.numeric(rep(1,length(IDc))),ns=as.numeric(ns),path=as.character(path),f$d[,1:(max(ns)-1)])
