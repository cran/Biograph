...
{ print ('check.par: the function checks whether "format.date" is present (attribute of Biograph object) and all parameters are defined (by function Parameters) ')
  ...
  if (exists ("namstates")) namstates3 <- namstates else namstates3  <- StateSpace (Bdata)$namstates
  if (!exists("namstates")| !exists ("iagehigh") | !exists ("maxtrans")) z<- Parameters (Bdata) else z <- "OK"
