Sequences <-
function (survey) {
# TASK: From SURVEY.DAT (which is "survey"), determine number of different sequences
#    The main variable used is survey$path
#    called from eg GLHS\survey\survey3.r
# input :  survey  (= SURVEY.DAT) and ages (produced in AGE_TRANS.r)
# Output : sequences_sorted  (through RETURN)
#
# ntcase = number of different sequences
# case(ntcase) = sequences based on survey$path
...
# n_match = survey$path matches the n_match-th element of vector case
# mages = mean ages at transitions in different pathtypes

agetrans <- AgeTrans(survey)
ages <- agetrans$ages
ageentry <- agetrans$ageentry
agecens <- agetrans$agecens
ncase <- sort(table(survey$path),decreasing=TRUE)  # number of states in this path
case <- names(ncase)  # same as: case <- unique(survey$path)
case_ind <- match(survey$path,case)
ns_case <- nchar(case)

# Mean age at transition and at censoring (M_age_int), by path type
mages <- aggregate(ages,by=list(case_ind),mean,na.rm=TRUE)
mages <- round(mages,2)
mageentry <- round(aggregate(ageentry,by=list(case_ind),mean,na.rm=TRUE),2)
magecens <- round(aggregate(agecens,by=list(case_ind),mean,na.rm=TRUE),2)
sequences <-data.frame(cbind(case,ns_case,ncase,mageentry[,-1],magecens[,-1],mages[,-1]))
colnames(sequences) <- c("case","ns","ncase","M_age_entry","M_age_exit",paste("tr",1:(ncol(survey)-locpath(survey)),sep="")) # -1 for cohort added dec 09
...
sequences_sorted <- array(" ",c(nrow(sequences2),(ncol(survey)-locpath(survey))))     # number of transitions !! 
# Add to each mean age at transition, the destination state (state occupied after transition) 
...
# Arrange the columns in dataframe as in SURVEYLIFE
